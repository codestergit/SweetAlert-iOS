...
class LoadingAnimatedView: AnimatableView{
    ...
    private var circlePath: CGPath {
        ...
        let endAngle: CGFloat = CGFloat(((2.0*M_PI) as! NSNumber).floatValue)
    ...
    private func setupLayers(){
        ...
        CATransaction.begin()
        circleLayer.strokeStart=0.25
        CATransaction.commit()
    }
    override func animate(){
        let animation=CABasicAnimation(keyPath: "transform.rotation")
        animation.duration=2.0
        animation.byValue=NSNumber(value: 2*M_PI )
        ...
        circleLayer.add(groupAnimations, forKey: "drawAnimation")
        circleLayer.add(animation, forKey: "rotateAnimation")
    }
}