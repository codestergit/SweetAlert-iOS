...
/**
 Apple:Enumerations defined in Swift
...
@objc open class SweetAlertObjc: UIViewController {
    ...
    let kFontLight = "HelveticaNeue-Light"
    ...
    fileprivate func setupTitleLabel() {
        ...
        titleLabel.textColor = UIColor.colorFromRGB(0x3f3f3f)
    }
    
    fileprivate func setupSubtitleTextView() {
        subTitleTextView.text = ""
        subTitleTextView.textAlignment = .center
        subTitleTextView.font = UIFont(name: kFontLight, size:16)
        subTitleTextView.textColor = UIColor.colorFromRGB(0x3f3f3f)
    ...
    fileprivate func resizeAndRelayout() {
        ...
        for i in 0 ..< buttons.count {
            ...
            buttons[i].layer.cornerRadius = 3.0
            self.contentView.addSubview(buttons[i])
            buttons[i].addTarget(self, action: #selector(SweetAlertObjc.pressed(_:)), for: UIControl.Event.touchUpInside)
    ...
    @objc func showAlert(_ title: String?, subTitle: String?, style: String,buttonTitle: String?, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlertObjc {
        if (buttonTitle != nil){
            ...
            _ = self.showAlert(title, subTitle: subTitle, style: style, buttonTitle: "OK" ,buttonColor:tealColor)
        }
        userAction = action
        return self
    }
    ...
    @objc func showAlert(_ title: String?, subTitle: String?, style: String,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        ...
        default:
            if let image = UIImage(named: style) {
        ...
        if buttonTitle.isEmpty == false {
            ...
            button.titleLabel?.font = UIFont(name: kFontLight, size:16);
        ...
        if otherButtonTitle != nil && otherButtonTitle!.isEmpty == false {
            ...
            button.addTarget(self, action: #selector(SweetAlertObjc.pressed(_:)), for: UIControl.Event.touchUpInside)
            button.tag = 1
            buttons.append(button)
        }
...
class CancelAnimatedView: AnimatableView {
    ...
    override func animate() {
        ...
        t2.m34 = 1.0 / -500.0;
        t2 = CATransform3DRotate(t2, -CGFloat.pi, 1, 0, 0);
